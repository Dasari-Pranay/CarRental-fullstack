...
const CarCard = ({ car }) => {
  ...
  return (
    <div
      ...
      className={`group rounded-xl overflow-hidden shadow-md hover:shadow-xl hover:bg-blue-50 hover:-translate-y-1 transition-all duration-500 cursor-pointer 
      ${!car.isAvailable ? 'opacity-70 pointer-events-none' : ''}`}
    >
      {/* Image Section */}
      <div className="relative h-48 overflow-hidden">
        ...
        {car.isAvailable ? (
          <p className="absolute top-4 left-4 bg-green-500 text-white text-xs px-2.5 py-1 rounded-full">
            Available Now
          </p>
        ) : (
          <p className="absolute top-4 left-4 bg-red-500 text-white text-xs px-2.5 py-1 rounded-full">
            Unavailable
          </p>
        )}
